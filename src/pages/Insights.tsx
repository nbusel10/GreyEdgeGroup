...
import { Btn, Container, Reveal, Section } from '../components/ui'
import { usePageMeta } from '../lib/meta'

const categories = ['All', 'Technical Education', 'Case Study', 'Industry and Policy'] as const
        ...
        <Container>
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {shown.map((item, i) => (
              <Reveal key={item.slug} delay={(i % 3) * 0.06}>
                ...
                >
                  <div className="h-48 overflow-hidden bg-ge-light">
                    <img
                      src={item.image}
                      alt={item.imageAlt}
                      loading="lazy"
          ...
          <Reveal className="mt-14">
            <div className="flex flex-col items-start justify-between gap-6 border border-ge-light bg-white p-8 md:flex-row md:items-center md:p-10">
              <div>
                <h2 className="font-display text-2xl font-bold uppercase tracking-wide text-ge-black">
                  More on LinkedIn
                </h2>
